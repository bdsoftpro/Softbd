...
class UserProfileTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->editPageForTheCurrentUser = route('softbd.users.edit', ['user' => $this->user->id]);

        $this->listOfUsers = route('softbd.users.index');
    }

    public function testCanSeeTheUserInfoOnHisProfilePage()
    {
        $this->visit(route('softbd.profile'))
    ...
    public function testCanEditUserPassword()
    {
        $this->visit(route('softbd.profile'))
             ...
             ->see('Edit User')
             ->seePageIs($this->editPageForTheCurrentUser)
             ->type('softbd-rocks', 'password')
             ->press('Submit')
             ->seePageIs($this->editPageForTheCurrentUser);

        $updatedPassword = DB::table('users')->where('id', 1)->first()->password;
        $this->assertTrue(Hash::check('softbd-rocks', $updatedPassword));
    }
