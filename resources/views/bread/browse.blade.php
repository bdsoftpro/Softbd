                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        <td>
                                            ...
                                            @elseif($row->type == 'text')
                                                @include('softbd::multilingual.input-hidden-bread-browse')
                                    ...
                                    <td class="no-sort no-click" id="bread-actions">
                                        @if (Softbd::can('delete_'.$dataType->name))
                                            <a href="javascript:;" title="Delete" class="btn btn-sm btn-danger pull-right delete" data-id="{{ $data->id }}" id="delete-{{ $data->id }}">
                                                <i class="softbd-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
                                            </a>
                                        @endif
                                        @if (Softbd::can('edit_'.$dataType->name))
                                            <a href="{{ route('softbd.'.$dataType->slug.'.edit', $data->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="softbd-edit"></i> <span class="hidden-xs hidden-sm">Edit</span>
                                            </a>
                                        @endif
                                        @if (Softbd::can('read_'.$dataType->name))
                                            <a href="{{ route('softbd.'.$dataType->slug.'.show', $data->id) }}" title="View" class="btn btn-sm btn-warning pull-right">
                                                <i class="softbd-eye"></i> <span class="hidden-xs hidden-sm">View</span>
                                            </a>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="softbd-trash"></i> Are you sure you want to delete
...
<link rel="stylesheet" href="{{ softbd_asset('lib/css/responsive.dataTables.min.css') }}">
@endif
@stop
...
@section('javascript')
    <!-- DataTables -->
    @if(!$dataType->server_side && config('dashboard.data_tables.responsive'))
        <script src="{{ softbd_asset('lib/js/dataTables.responsive.min.js') }}"></script>
    @endif
