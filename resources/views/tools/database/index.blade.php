...
@section('css')
    <script type="text/javascript" src="{{ softbd_asset('js/vue21.min.js') }}"></script>
    <link rel="stylesheet" href="{{ softbd_asset('css/database.css') }}">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="softbd-data"></i> Database
        <a href="{{ route('softbd.database.create') }}" class="btn btn-success"><i class="softbd-plus"></i>
            Create New Table</a>
    </h1>
@stop
    ...
    <div class="page-content container-fluid">
        ...
        <div class="row">
            ...
            @foreach($tables as $table)
                    @continue(in_array($table->name, config('softbd.database.tables.hidden', [])))
                    <tr>
                        <td>
                            <p class="name">
                                <a href="{{ route('softbd.database.show', $table->name) }}"
                                   data-name="{{ $table->name }}" class="desctable">
                                   {{ $table->name }}
                                </a>
                            @if($table->dataTypeId)
                                <i class="softbd-bread"
                        ...
                        <td>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('softbd.database.bread.edit', $table->name) }}"
                            ...
                            @else
                                <a href="{{ route('softbd.database.bread.create', ['name' => $table->name]) }}"
                                   class="btn-sm btn-default">
                                    <i class="softbd-plus"></i> Add BREAD to this table
                                </a>
                            @endif
                            </div>
                        </td>
                        ...
                        <td class="actions">
                            ...
                            <a href="{{ route('softbd.database.edit', $table->name) }}"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="softbd-trash"></i> Are you sure you want to delete the BREAD for
                        the <span id="delete_builder_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softbd.database.bread.delete', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softbd-trash"></i> Are you sure you want to delete the <span
                                id="delete_table_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softbd.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="softbd-data"></i> @{{ table.name }}</h4>
                </div>
