...
@section('css')
    <style>
        .panel-actions .softbd-trash {
            cursor: pointer;
        }

        .panel-actions .softbd-trash:hover {
            color: #e94542;
        }
...
@section('head')
    <script type="text/javascript" src="{{ softbd_asset('lib/js/jsonarea/jsonarea.min.js') }}"></script>
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="softbd-settings"></i> Settings
    </h1>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        ...
        @if(config('softbd.show_dev_tips'))
        <div class="alert alert-info">
            <strong>How To Use:</strong>
            <p>You can get the value of each setting anywhere on your site by calling <code>Softbd::setting('key')</code></p>
        </div>
        @endif
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('softbd.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Softbd::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('softbd.settings.move_up', $setting->id) }}">
                                <i class="sort-icons softbd-sort-asc"></i>
                            </a>
                            <a href="{{ route('softbd.settings.move_down', $setting->id) }}">
                                <i class="sort-icons softbd-sort-desc"></i>
                            </a>
                            <i class="softbd-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('softbd.storage.disk'))->exists($setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('softbd.settings.delete_value', $setting->id) }}" class="softbd-x"></a>
                                    <img src="{{ Storage::disk(config('softbd.storage.disk'))->url($setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="softbd-plus"></i> New Setting</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('softbd.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="softbd-double-down"></i> OPTIONS</a>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .softbd-double-down').length) {
                                    $('#toggle_options .softbd-double-down').removeClass('softbd-double-down').addClass('softbd-double-up');
                                } else {
                                    $('#toggle_options .softbd-double-up').removeClass('softbd-double-up').addClass('softbd-double-down');
                                }
                            });
                        });
                    </script>
                    ...
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="softbd-plus"></i> Add New Setting
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="softbd-trash"></i> Are you sure you want to delete the <span id="delete_setting_title"></span> Setting?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softbd.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('softbd.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
        <input name="image" id="upload_file" type="file" onchange="$('#my_form').submit();this.value='';">
        <input type="hidden" name="type_slug" id="type_slug" value="settings">
    </form>

    <script src="{{ softbd_asset('lib/js/tinymce/tinymce.min.js') }}"></script>
    <script src="{{ softbd_asset('js/softbd_tinymce.js') }}"></script>
    <script src="{{ softbd_asset('lib/js/ace/ace.js') }}"></script>
    <script src="{{ softbd_asset('js/softbd_ace_editor.js') }}"></script>
    <script>
