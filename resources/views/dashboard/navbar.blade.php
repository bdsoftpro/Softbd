<nav class="navbar navbar-default navbar-fixed-top navbar-top">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a id="sidebar-anchor" class="softbd-anchor btn-link navbar-link hidden-xs" 
            ...
            <ol class="breadcrumb hidden-xs">
                @if(count(Request::segments()) == 1)
                    <li class="active"><i class="softbd-boat"></i> Dashboard</li>
                @else
                    <li class="active">
                        <a href="{{ route('softbd.dashboard')}}"><i class="softbd-boat"></i> Dashboard</a>
                    </li>
                @endif
                <?php $breadcrumb_url = ''; ?>
                @for($i = 1; $i <= count(Request::segments()); $i++)
                    <?php $breadcrumb_url .= '/' . Request::segment($i); ?>
                    @if(Request::segment($i) != ltrim(route('softbd.dashboard', [], false), '/') && !is_numeric(Request::segment($i)))
        ...
        <ul class="nav navbar-nav navbar-right">
            <li class="dropdown profile">
                ...
                <ul class="dropdown-menu dropdown-menu-animated">
                    ...
                    <li class="divider"></li>
                    <?php $nav_items = config('softbd.dashboard.navbar_items'); ?>
                    @if(is_array($nav_items) && !empty($nav_items))
                    @foreach($nav_items as $name => $item)
                    <li {!! isset($item['classes']) && !empty($item['classes']) ? 'class="'.$item['classes'].'"' : '' !!}>
                        @if(isset($item['route']) && $item['route'] == 'softbd.logout')
                        <form action="{{ route('softbd.logout') }}" method="POST">
                            {{ csrf_field() }}
