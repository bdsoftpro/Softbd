...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ softbd_asset('css/nestable.css') }}">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="softbd-list"></i>Menu Builder ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="softbd-plus"></i> New Menu Item</div>
    </h1>
    @include('softbd::multilingual.language-selector')
@stop
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="softbd-trash"></i> Are you sure you want to delete this menu item?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('softbd.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="softbd-plus"></i> Create a New Menu Item</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="softbd-edit"></i> Edit Menu Item</h4>
                </div>
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('softbd.menus.item.add', ['menu' => $menu->id]) }}"
                      data-action-update="{{ route('softbd.menus.item.update', ['menu' => $menu->id]) }}">
                    ...
                    <div class="modal-body">
                        ...
                        @include('softbd::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                        ...
                        <label for="icon_class">Font Icon class for the Menu Item (Use a <a
                                    href="{{ softbd_asset('fonts/softbd/icons-reference.html') }}"
                                    target="_blank">Softbd Font Class</a>)</label>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ softbd_asset('js/jquery.nestable.js') }}"></script>
    @if($isModelTranslatable)
        <script type="text/javascript" src="{{ softbd_asset('js/multilingual.js') }}"></script>
    @endif
    <script>
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('softbd.menus.order',['menu' => $menu->id]) }}', {
