...
use SBD\Softbd\Models\Translation;
use SBD\Softbd\Translator;
...
trait Translatable
{
    ...
    public function translations()
    {
        return $this->hasMany(Translation::class, 'foreign_key', $this->getKeyName())
            ...
            ->whereIn('locale', config('softbd.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('softbd.multilingual.locales', [config('softbd.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('softbd.multilingual.default'), false];
        }
        ...
        $default = config('softbd.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('softbd.multilingual.default', 'en');
        $locales = config('softbd.multilingual.locales', [$default]);
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('softbd.multilingual.default', 'en')]]);
