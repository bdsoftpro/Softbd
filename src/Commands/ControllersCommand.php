...
class ControllersCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'softbd:controllers';
     ...
     * @var string
     */
    protected $description = 'Publish all the controllers from Softbd.';
    ...
    {
        $stub = $this->getStub();
        $files = $this->filesystem->files(base_path('vendor/sbd/softbd/src/Http/Controllers'));
        $namespace = config('softbd.controllers.namespace', 'SBD\\Softbd\\Http\\Controllers');
        ...
        $this->info('Published Softbd controllers!');
    }
    ...
    {
        return $this->filesystem->get(base_path('/vendor/sbd/softbd/stubs/'.$this->stub));
    }
    ...
    protected function generateContent($stub, $class)
    {
        ...
        $content = str_replace(
            'FullBaseDummyClass',
            'SBD\\Softbd\\Http\\Controllers\\'.$class,
            $content
        );
