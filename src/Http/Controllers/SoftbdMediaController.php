...
class SoftbdMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('softbd.storage.disk');
    }

    public function index()
    {
        Softbd::canOrFail('browse_media');

        return view('softbd::media.index');
    }
    ...
    private function getFiles($dir)
    {
        ...
        foreach ($storageFiles as $file) {
            if (empty(pathinfo($file, PATHINFO_FILENAME)) && config('softbd.hidden_files')) {
                $files[] = [
