...
abstract class Controller extends BaseController
{
        ...
        switch ($row->type) {
            ...
            case 'file':
                if ($file = $request->file($row->field)) {
                    ...
                    $request->file($row->field)->storeAs(
                        ...
                        config('softbd.storage.disk', 'public')
                    );
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    Storage::disk(config('softbd.storage.disk'))->put($fullPath, (string) $image, 'public');
                    ...
                    if (isset($options->thumbnails)) {
                        foreach ($options->thumbnails as $thumbnails) {
                            ...
                            Storage::disk(config('softbd.storage.disk'))->put($path.$filename.'-'.$thumbnails->name.'.'.$file->getClientOriginalExtension(),
                                (string) $image, 'public'
                            );
                        }
                    }
    ...
    {
        if (Storage::disk(config('softbd.storage.disk'))->exists($path)) {
            Storage::disk(config('softbd.storage.disk'))->delete($path);
        }
    }
}
