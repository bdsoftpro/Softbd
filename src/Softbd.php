...
use SBD\Softbd\FormFields\After\HandlerInterface as AfterHandlerInterface;
use SBD\Softbd\FormFields\HandlerInterface;
use SBD\Softbd\Models\Category;
use SBD\Softbd\Models\DataRow;
use SBD\Softbd\Models\DataType;
use SBD\Softbd\Models\Menu;
use SBD\Softbd\Models\MenuItem;
use SBD\Softbd\Models\Page;
use SBD\Softbd\Models\Permission;
use SBD\Softbd\Models\Post;
use SBD\Softbd\Models\Role;
use SBD\Softbd\Models\Setting;
...
class Softbd
{
    ...
    {
        if (!empty($file) && Storage::disk(config('softbd.storage.disk'))->exists($file)) {
            return Storage::disk(config('softbd.storage.disk'))->url($file);
        }
    ...
    {
        require __DIR__.'/../routes/softbd.php';
    }
    ...
    public function alerts()
    {
        if (!$this->alertsCollected) {
            event('softbd.alerts.collecting');
    ...
    protected function findVersion()
    {
        ...
        if ($this->filesystem->exists(base_path('composer.lock'))) {
            ...
            // Loop through all the packages and get the version of softbd
            foreach ($file->packages as $package) {
                if ($package->name == 'sbd/softbd') {
    ...
    {
        if (!config('softbd.multilingual.bread')) {
            return false;
        }
